...
public class ContaDAOPostgres implements ContaDAO {
    ...
    public void inserirConta(Account conta) {
        ...
        try {
            String sql = "INSERT INTO t_conta (id, numero, saldo, tipo, pessoa_id) VALUES (?,?,?,CAST(? as tipo_conta) ,?) ";
    ...
    public Account buscarContaPorDocumentoTitular(String documento) {
        try {
            String sql = "SELECT c.id, c.numero, c.saldo, c.tipo, c.pessoa_id,  p.nome, p.tipo as pessoa_tipo FROM t_conta as c JOIN t_pessoa as p ON c.pessoa_id = p.id WHERE p.documento =  ?";
            PreparedStatement stm = this.connection.prepareStatement(sql);
            ...
            if (resp.next()) {
                ...
                switch (tipoConta) {
                    case "corrente":
                        return new ContaCorrente(idConta, pessoa, numero, tipoConta, tipo, saldo);
                    case "salario":
                        return new ContaSalario(idConta, pessoa, numero, tipoConta, tipo, saldo);
                    default:
                        return new ContaPoupanca(idConta, pessoa, numero, tipoConta, tipo, saldo);
                }
            } 
        } catch (SQLException e) {
            System.out.println(e.getMessage());
